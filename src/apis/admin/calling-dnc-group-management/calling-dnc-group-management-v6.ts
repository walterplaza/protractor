...
export default class CallingDNCGroupManagementVersion6 implements ICallingDNCGroupManagement {
    ...
    async returnsAllCallingLists(agent: Agent): Promise<APIResponse> {
        try {
            let options = new Options(`${agent.baseUri}services/v6.0/lists/call-lists`, Method.GET);
    ...
    async uploadNewRecordsToACallList(agent: Agent, listId: any, skillId: any, fileName: any, listFile: any, forceOverwrite: any = "false"): Promise<APIResponse> {
        try {
            let options = new Options(`${agent.baseUri}services/v6.0/lists/call-lists/${listId}/upload`, Method.POST);
            options.addHeader("Authorization", `${agent.tokenType} ${agent.accessToken}`);
            options.addHeader("Content-Type", "application/json; charset=utf-8");
