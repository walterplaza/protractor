...
export default class PersonalConnectionManagementVersion2 implements IPersonalConnectionManagement {
    async logIntoADialerCampaign(agent: Agent, skillName: any): Promise<APIResponse> {
        try {
            let options = new Options(`${agent.baseUri}services/v2.0/agent-sessions/${agent.sessionId}/dialer-login`, Method.POST);
    ...
    async logOutOfADialerCampaign(agent: Agent): Promise<APIResponse> {
        try {
            let options = new Options(`${agent.baseUri}services/v2.0/agent-sessions/${agent.sessionId}/dialer-logout`, Method.POST);
            options.addHeader("Authorization", `${agent.tokenType} ${agent.accessToken}`);
            options.addHeader("Content-Type", "application/json; charset=utf-8");
            return await APICore.request(options);
        } catch (err) {
