...
export default class NewReportAPIHTML {
    static generateSummaries = function (reportXml, report) {
        ...
        for (var i = 0; i < totalSuites; i++) {
            ...
            for (let j = 0; j < totalCasesPerSuite; j++) {
                let testCasesInSuite = testCases[j].childrenNamed('testcase')
                ...
                if (testCases[j].childrenNamed('failure').length == 0) {
                    ...
                    for (let n = 0; n < failureT.length; n++) {
                        ...
                        if(tempReportMess != ""){
                            tempReportMess = `${ConfigReport.getBugErrorMessage(testCaseId, tempReportMess)}. ` + failureT[n].attr.message;
                        }// tempReportMess += failureT[n].attr.message + ConfigReport.getBugErrorMessage(testCaseId);
                    }
                ...
                if (testCasesResults[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
                    ...
                    if (report.modifiedSuiteName) {
                        ...
                        screenshotsNamesOnFailure.push(suite.name.substr(suite.name.length - 7, 7).trim() + '.png');
                    }
                }
            }
