...
describe("UserHub- IC-63290", function () {
    ...
    beforeEach(async () => {
        await Logger.write(FunctionType.TESTCASE, `IC-63290 - [MAX][UserHub][Notification][Evaluation Acknowledge] Launch button will open the Evolve Panel and redirects to the given URL`)
        admin = await TestCondition.setUpUserHubAgent(AgentType.USERHUB_OBPHONE);
    }, TestRunInfo.conditionTimeout);

    it('IC-63290 - MAX UserHub Notification Evaluation Acknowledge Launch button will open the Evolve Panel and redirects to the given URL', async () => {
        ...
        // VP: Acknowledge button is displayed
        expect(await qmFormPage.isAcknowledButtonDisplayed()).toBe(true, "Acknowledge button is not displayed");
    ...
    afterEach(async () => {
        ...
        try {
            ...
            // Close QM Form
            evaluationsPage = await qmFormPage.closeQMForm();
