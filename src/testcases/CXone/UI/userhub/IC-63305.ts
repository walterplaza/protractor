...
describe('UserHub - IC-63305', function () {
    ...
    beforeEach(async () => {
        await Logger.write(FunctionType.TESTCASE, `IC-63305 - [MAX][UserHub][Notification][Decline request] Launch button will open the Evolve Panel and redirects to the given URL`);
        admin = await TestCondition.setUpUserHubAgent(AgentType.USERHUB_OBPHONE);
        manager = await TestCondition.setUpUserHubAgent(AgentType.USERHUB_IBPHONE);
        ...
        // - As an Administrator create a new Time Off Request (My Zone> My Time Off Requests> New Request). -- This will generate a new Request with 'Pending" Status
        ...
        // // - As a Manager Decline the Agent's Request (Scheduler> Requests> <look for the 'New' Request> ) 
        // // - BU must not have persistent panels configured 
        ...
        await requestsPage.declineRequest(admin.email.toLowerCase());

    }, TestRunInfo.conditionTimeout);

    it('IC-63305 -  MAX UserHub Notification Decline request Launch button will open the Evolve Panel and redirects to the given URL', async () => {
        // //2. Click on Messages
        ...
        // //3. Look for Time Off Request Declined Notification
