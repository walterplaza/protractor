...
describe('max-userhub-brandembassy - IC-107741', function () {
    ...
    beforeEach(async () => {
        await Logger.write(FunctionType.TESTCASE, 'IC-107741 - [Patron][BE]Verify that BE is displayed once refresh MAX');
        // Preconditions
    ...
    it('IC-107741 - [Patron][BE]Verify that BE is displayed once refresh MAX', async () => {
        //Launch MAX
        ...
        // VP: Check Digital engagement link is displayed on MAX
        ...
        //VP:Close BE workspace and check that BE workspace is not visible
        await maxPage.clickBECustomWorkspaceCloseButton();
        expect((await maxPage.getWFOWorkspaceVisibility()).includes("hidden")).toBe(true, "Digital Engagement web page/workspace is not closed or the WFO workspace is not made hidden");
        ...
        await maxPage.clickBECustomWorkspaceCloseButton(); // BE custom workspace is overlayed on the MAX glace, without closing it cannot Logout MAX
