...
describe("MAX suite - IC-101289", function () {
    ...
    beforeEach(async () => {
        await Logger.write(FunctionType.TESTCASE, `IC-101289 - MAX > Email > Inbound email interrupted by call - save reply draft progress and send`);
    ...
    it('IC-101289 - MAX Email Inbound email interrupted by call - save reply draft progress and send', async () => {
        ...
        expect(await activeContactsPage.isActiveContactDisplayed(emailContactId, ContactState.ACTIVE, TestRunInfo.shortTimeout)).toBe(true, "The report about the IB Email contact is not Interrupted");
