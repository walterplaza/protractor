...
let testCaseName: string = "Returns Agents assigned to a Skill";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let chatAgent: Agent;
    let apiName: string = "returns-agents-assigned-to-a-skill";
    let skillID: number;
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                let res: APIResponse = await apiClass.returnsAgentsAssignedToASkill(chatAgent, skillID);
