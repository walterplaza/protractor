...
let testCaseName: string = "Allows to begin the recording of an active phone call";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let ibPhone: Agent;

    // Read Data
    let dataFullTest = Utility.readJsonAPI(`Admin/general-contact-management/allows-to-begin-the-recording-of-an-active-phone-call/allows-to-begin-the-recording-of-an-active-phone-call-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            // Create random phoneNumber
            await ibPhone.createPhoneNumber(); 

            for (let caseData of testCaseData.Data) {
                 let res: APIResponse = await apiClass.allowsToBeginTheRecordingOfAnActivePhoneCall(ibPhone, contactId);
