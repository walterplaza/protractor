...
let testCaseName: string = "Starts monitoring a phone call";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let ibPhone: Agent;

    // Read Data
    let dataFullTest = Utility.readJsonAPI(`Admin/general-contact-management/starts-monitoring-a-phone-call/starts-monitoring-a-phone-call-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                 let res: APIResponse = await apiClass.startsMonitoringAPhoneCall(ibPhone,contactId, ibPhone.phoneNumber);
