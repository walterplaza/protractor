...
let testCaseName: string = "Returns possible Timezones";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let agent: Agent;

    // Read Data
    let dataFullTest = Utility.readJsonAPI(`admin/general-admin-management/returns-possible-timezones/returns-possible-timezones-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                ...
                let res: APIResponse = await apiClass.returnsPossibleTimezones(agent);
