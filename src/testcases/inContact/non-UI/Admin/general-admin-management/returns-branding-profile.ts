...
let testCaseName: string = "Returns Branding Profile";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let agent: Agent;
    let bussinessId: number;

    // Read Data
    let dataFullTest = Utility.readJsonAPI(`admin/general-admin-management/returns-branding-profile/returns-branding-profile-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            bussinessId = parseInt(agent.businessNumber);
            for (let caseData of testCaseData.Data) {
                let res: APIResponse = await apiClass.returnsBrandingProfile(agent,bussinessId);
