...
let testCaseName: string = "Create or Update Dynamic Address Book Entries";
...
describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    ...
    // Read Data
    let dataFullTest = Utility.readJsonAPI(`Admin/address-book-management/create-or-update-dynamic-address-book-entries/create-or-update-dynamic-address-book-entries-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                let res: APIResponse = await apiClass.createOrUpdateDynamicAddressBookEntries(chatAgent, addressBookId, caseData.QueryParams.fullLoad, externalId, "Test", "API", chatAgent.phoneNumber);
