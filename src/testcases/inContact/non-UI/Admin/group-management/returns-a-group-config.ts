...
let testCaseName: string = "Returns a group config";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {

    let chatAgent: Agent;
    let dataFullTest = Utility.readJsonAPI(`admin/group-management/returns-a-group-config/returns-a-group-config-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                let groupId: string = await CustomAPIs.getRandomGroupId(chatAgent);
                let res: APIResponse = await groupManagementAPI.returnsAGroupConfig(chatAgent, groupId);
