...
let testCaseName: string = "Add Records to a DNC Group";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let chatAgent: Agent;

    // Read Data
    let dataFullTest = Utility.readJsonAPI(`admin/calling-dnc-group-management/add-records-to-a-dnc-group/add-records-to-a-dnc-group-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                ...
                let res2: APIResponse = await apiClass.addRecordsToADncGroup(chatAgent,dncGroupId,chatAgent.phoneNumber,expireDate);
                expect(res2.status).toBe(caseData.Expected.statusCode, "Status code does not match expected");
                await apiClass.expireRecordsFromADncGroup(chatAgent,dncGroupId,chatAgent.phoneNumber,expireDate);
            }
        });
    })
})
