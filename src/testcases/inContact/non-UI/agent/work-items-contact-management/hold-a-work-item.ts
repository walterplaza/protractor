...
describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, async function () {
    ...
    let dataFullTest = Utility.readJsonAPI(`agent/work-items-contact-management/hold-a-work-item/hold-a-work-item-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                   if (testCaseData.Id == "AGV120046") {
                    let returnResponse: APIResponse = await apiClassWorkItem.holdAWorkItem(workitemAgent, contactID);
