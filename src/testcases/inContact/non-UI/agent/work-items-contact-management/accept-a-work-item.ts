...
let testCaseName: string = "Accept a work item";
...
describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, async function () {
    ...
    let dataFullTest = Utility.readJsonAPI(`agent/work-items-contact-management/accept-a-work-item/accept-a-work-item-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                 if (testCaseData.Id == "AGV120049") {
                    let returnResponse: APIResponse = await apiClassWorkItem.acceptAWorkItem(workitemAgent, contactID);
