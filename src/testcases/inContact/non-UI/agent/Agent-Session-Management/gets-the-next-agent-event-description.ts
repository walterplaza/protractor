...
let testCaseName: string = "Gets the next agent event description";
...
describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    ...
    let dataFullTest = Utility.readJsonAPI(`agent/agent-session-management/gets-the-next-agent-event-description/gets-the-next-agent-event-description-${TestRunInfo.versionAPI}.json`);
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                if (testCaseData.Id == "AGV120057") {
                    let returnResponse: APIResponse = await apiClass.getsTheNextAgentEventDescription(chatAgent, 20);
