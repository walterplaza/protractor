...
let testCaseName: string = "Send DTMF tones";

describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    let agent: Agent;

    let dataFullTest = Utility.readJsonAPI(`agent/phone-call-management/send-dtmf-tones/send-dtmf-tones-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                ...
                let res: APIResponse = await phoneCallManagementAPI.sendDtmfTones(agent, caseData.QueryParams.DtmfSequence, caseData.QueryParams.ToneDurationMS);                
