...
let testCaseName: string = "Cancels a presented scheduled callback";
...
describe(`${testCaseName} - ${TestRunInfo.versionAPI}`, function () {
    ...
    // Read Data
    let dataFullTest = Utility.readJsonAPI(`agent/scheduled-callbacks-management/cancels-a-presented-scheduled-callback/cancels-a-presented-scheduled-callback-${TestRunInfo.versionAPI}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${TestRunInfo.versionAPI}`, async function () {
            ...
            callbackId = await CustomAPIs.waitForNextEventActive(obPhoneAgent, waitTime);

            for (let caseData of testCaseData.Data) {
                if (testCaseData.Id == "AGV120053") {
                    let returnResponse: APIResponse = await agentScheduleApiClass.cancelsAPresentedScheduledCallback(obPhoneAgent, callbackId);
