...
describe(`${testCaseName} - ${APIVersion.V12}`, async function () {
    ...
    // Read Data
    let dataFullTest = Utility.readJsonAPI(`reporting/historical-reporting/return-scorecard-statistics/return-scorecard-statistics-${APIVersion.V12}.json`);
    ...
    dataFullTest.map(function (testCaseData) {
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${APIVersion.V12}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                ...
                let response: APIResponse = await historicalReportingAPI.returnScorecardStatistics(agent, startDate, endDate);
