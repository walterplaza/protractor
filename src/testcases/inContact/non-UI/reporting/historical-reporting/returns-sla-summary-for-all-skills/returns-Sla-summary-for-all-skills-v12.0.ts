...
describe(`${testCaseName} - ${APIVersion.V12}`, async function () {
    ...
    let dataFullTest = Utility.readJsonAPI(`reporting/historical-reporting/returns-sla-summary-for-all-skills/returns-sla-summary-for-all-skills-${APIVersion.V12}.json`);
        ...
        it(`${testCaseData.Id} - ${testCaseName} ${testCaseData.Description} ${APIVersion.V12}`, async function () {
            ...
            for (let caseData of testCaseData.Data) {
                if (testCaseData.Id == "RPV120016") {
                    let res = await reportingApis.returnsSlaSummaryForAllSkills(chatAgent, caseData.QueryParams.startDate, caseData.QueryParams.endDate);
